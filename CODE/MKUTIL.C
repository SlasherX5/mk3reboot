...
void back_to_normal(void)
{
	...
	current_proc->pa8->oflags2 &=~(M_NOFLIP|M_NOBLOCK|M_NOCOL|M_NOEDGE|M_HALF_DAMAGE);              // clear flip & block flags
...
void advance_him(void)
{
	...
	do_next_a9_frame((OBJECT *)ptemp->pa10);                         // use other guys object
...
/******************************************************************************
 Function:  void inc_his_p_hit(void)
...
void clr_scrn(void)
{
	...
	cdb = &db;//(cdb==db)? db+1: db;                                     /* swap double buffer ID */
...
/******************************************************************************
 Function:  void center_about_x(OBJECT *obj,short xpos)
...
******************************************************************************/
 void center_about_x(OBJECT *obj,short xpos)
{
...
/******************************************************************************
 Function:  void no_edge_limits(void)
...
/******************************************************************************
 Function:  WORD is_he_airborn(void)
...
/******************************************************************************
 Function:  int get_my_height(void)
...
******************************************************************************/
 WORD is_he_right(PROCESS *ptemp)
{
...
void fastxfer(PROCESS *pxfer,JUMPTBL jumpptr)
{
	...
	InitProcessStack(pxfer); //  reset process stack
	pxfer->p_context.c_wakeup=jumpptr; // set wakeup point
	pxfer->p_comp_flag=PC_CLEAR; // addition to fix bug of xfer in middle 
								 // of move
	return;
}
...
/******************************************************************************
 Function:  void half_x_vel(OBJECT *obj)
...
void gravity_ani(WORD anispeed,long inity,long gravity)
{
	...
	{
		(ADDRESS *)current_proc->pdata.v_store4=(ADDRESS *)current_proc->a6;                    // save routine to call every
	...
	else
	{
		...
		if ((ADDRESS)current_proc->a6!=GNULL)
			(ADDRESS *)current_proc->pdata.v_store4=(ADDRESS *)current_proc->a6;            // save routine to call
...
******************************************************************************/
 WORD hit_ground_yet(OBJECT *obj)
{
...
{
	if (am_i_shang(current_proc->pa8)==SYSTEM_CARRY_CLR || (current_proc->pa8)->ochar==FT_ST)
		return;
...
{
	if (p1_obj==pa8 && p1_char==FT_ST)              // check if orig ST
		return(SYSTEM_CARRY_SET);

	if (p2_obj==pa8 && p2_char==FT_ST)              // check if orig ST
		return(SYSTEM_CARRY_SET);

	return(SYSTEM_CARRY_CLR);                               // nope
}
...
extern short ochar_ground_offsets[2][32];
...
void back_to_shang_form(void)
{
	...
	obj->oypos.u.intpos+=(ochar_ground_offsets[0][ochar]-ochar_ground_offsets[0][FT_ST]);
