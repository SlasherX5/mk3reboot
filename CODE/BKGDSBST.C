...
void calla_subway(void)
{
	...
	baklst4=(OBJECT *)current_proc->pa10=obj;	// turn lights on, enable list

	obj=get_lit_list(unlit_object_table);
	(OBJECT *)current_proc->pa11=obj;			// turn lights off,
...
OBJECT *get_lit_list(SLITE *table)
{
	...
	(OBJECT *)current_proc->pa0=obj_free;
	oa9=NULL;
	while (TRUE)
	{
...
TRAIN7:
	...
	{
		(OBJECT *)current_proc->pa1=obj;		// walk the list
		obj=obj->olink;
	}
...
******************************************************************************/
#if 0
extern void *mini_anims[];
extern void *ochar_mini_falls[];
extern void *ochar_mini_stunned[];
#endif
