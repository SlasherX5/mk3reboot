...
/* local externs */
extern void *kanoend_txt[];
extern void *sonyaend_txt[];
extern void *jaxend_txt[];
extern void *indianend_txt[];
extern void *subzeroend_txt[];
extern void *swatend_txt[];
extern void *liaend_txt[];
extern void *robo1end_txt[];
extern void *robo2end_txt[];
extern void *laoend_txt[];
extern void *tuskend_txt[];
extern void *sheevaend_txt[];
extern void *shangend_txt[];
extern void *lkangend_txt[];
extern void *smokeend_txt[];
extern void *godsend_txt[];

static void *end_texture_addr[]=
{
	"end\\kanoend.bin",    // kanoend_txt,
	"end\\sonyaend.bin",   // sonyaend_txt,
	"end\\jaxend.bin", 	  // jaxend_txt,
	"end\\indyend.bin",	  // indianend_txt,
	"end\\subend.bin",     // subzeroend_txt,
	"end\\swatend.bin",    // swatend_txt,
	"end\\liaend.bin",     // liaend_txt,
	"end\\robo1end.bin",   // robo1end_txt,
	"end\\robo2end.bin",   // robo2end_txt,
	"end\\laoend.bin",     // laoend_txt,
	"end\\tuskend.bin",    // tuskend_txt,
	"end\\shevaend.bin",   // sheevaend_txt,
	"end\\shangend.bin",   // shangend_txt,
	"end\\lkangend.bin",   // lkangend_txt,
	"end\\smokeend.bin",   // smokeend_txt
};

/******************************************************************************
 Function: void show_page_o_the_godz(void)
...
void show_page_o_the_godz(void)
{
	...
	load_bkgd=BKGD_END_GODS;								// load gods screen
	init_background_module(table_o_mods[load_bkgd]);

//  vram_texture_load(bkgd_base_x,BACK_TEXTURE_BASE_Y,(ADDRESS *)godsend_txt);
	// place textures in memory
	LoadBinToVram("end\\godsend.bin",bkgd_base_x,BACK_TEXTURE_BASE_Y);

	multi_plane();

	pds_centered_soff(txt_gods,SCX(0xc7),SCY(0xa6));
...
/******************************************************************************
 Function: void show_ending_story(WORD pa0)

 By: David Schwartz

 Date: May 1995

 Parameters: pa0 - winner

 Returns: None

 Description:	lets find out what happens to our winner!
******************************************************************************/
extern void *dlists_end1[];
...
void KLUGE_fadein_jsrp(WORD pa10)
{
	PROCESS *ptemp;
...
//	DISPLAY_ON;
	...
	process_sleep(1); //WIN95: PREVENT SCREEN FLASH (3) to work!!!!
//	DISPLAY_ON;

	if ( pa10 > 2 )
		fade_sleep_a0(pa10-2);
...
void smoke_page(void)
{
	...
	while(--(current_proc->a9)>0 );

	//pds_centered(txt_end_gameover,SCX(0xc8),SCY(0x10)+16);

	DISPLAY_OFF;
	KLUGE_fadein_jsrp(10);			// pause after fade
	process_sleep(1);
	DISPLAY_ON;
...
//LOAD SHAO DATA
	...
	psxcd_stop();	//WIN95: Added to stop CD 
	...
	show_ending_story(get_winner_ochar);		// music loaded on ending screen SCA3(0x21)
...
{
	(char *)current_proc->pa0=get_winner_text();
...
void mk3_design_team(DTEAM *dptr)
{
	...
	do
	{
		...
		while ( dptr->x1!=-1 )
		{
			...
			(WORD *)ptemp->pa11=team_fade_in;
			ptemp->a9=findpal(pal);
