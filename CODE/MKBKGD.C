...
void* bakmods[8] = { 0 };			/* array of ptrs to background modules */

//WIN95: Compressed temple hack
extern BYTE *back_uncompress_image(BYTE *src);
...
//WIN95	process_sleep(1);
// THIS WAS CAUSEING THE SCREEN TO FLASH 
...
HACK:
		...
		if ( results==0 )
		{
			//WIN95: p2_heap has been replaced by an indrection to bkgd_texture_loadaddr
			entry=GET_LONG(bkgd_texture_loadaddr);
			if (entry>7)
			{
				bkgd_preload=0;
				goto HACK;
			...
			//WIN95: p2_heap has been replaced by an indrection to bkgd_texture_loadaddr
			vram_texture_load(tsx,tsy,(ADDRESS *)bkgd_texture_loadaddr);
		}
...
void vram_texture_load(WORD tsx,WORD tsy,ADDRESS *text_tbl)
{
	...
	{
		DWORD text_info_offset=GET_LONG(text_tbl)++;                      /* get ptr to textyre data */
		text_info=COMPUTE_ADDR(text_heap, text_info_offset);         // compute abs addr
		...
		if ( load_bkgd == 8)
		{
			...
			text_info=(void*)back_uncompress_image((BYTE *)text_info);
		}
		//#endif
...
extern WORD TRK_TRAIN[];
extern WORD TRK_COOLT[];
...
void background_blast(void)
{
  ...
  psxspu_start_cd_fade(FADE_TIME,0);
	...
	(OBJECT *)current_proc->pa10=obj;                        // a10 - 1st ceiling block
	...
	(OBJECT *)(current_proc->pdata.v_store1)=obj;
	...
	(OBJECT *)(current_proc->pdata.v_store2)=obj;
...
//***********
	...
	switch (curback)
	{
		case BKGD_STREET_MOD:
			play_generic_tune(TRK_TRAIN);
			break;
		case BKGD_ROOF_MOD:
		case BKGD_TOWER_MOD:
			play_generic_tune(TRK_COOLT);
			break;
	}
	...
	do
	{
		...
		do
		{
			...
			if (ta2.pos>SCY(0x30000))
				ta2.pos=SCY(0x30000); // ***** MAYBE NEEDS_PORTING
...
void scroll_a9(void)
{
	...
	do
	{
		pa9=(ADDRESS)current_proc->a9;
		scroll_to_x_a9();
		(ADDRESS)current_proc->a9=pa9;
		process_sleep(1);
	}
	while(1);
}
...
void scroller(void)
{
	...
	while (TRUE)
	{
		...
		process_sleep(SCROLL_SLEEP_TIME);                               /* nighty nite */
	}
...
void create_dumb_animators(DTBL_INITA *tbl,void *heap)
{
	...
	{
		current_proc->pa9=tbl->a_list;
		(void *)current_proc->pa5=COMPUTE_ADDR(heap,GET_LONG(current_proc->pa9)++);      // check for correctness
