...
/******************************************************************************
 Function: void createBLOOD_Proc(WORD boffset)
...
******************************************************************************/
void createBLOOD_Proc(WORD boffset)
{
...
void sawb3(void)
{
	...
	(OBJECT *)current_proc->pa11=current_proc->pa8;
...
void spider_blood(void)
{
	...
	do
	{
		if ((current_proc->pdata.p_otherguy)->ochar==FT_SMOKE)
		{
...
{
	current_proc->pa11=current_proc->pa8;
	(long)current_proc->a0=SCX(0x20000);		// xvel range
	(long)current_proc->a1=SCX(0x10000);		// xvel min
	(long)current_proc->a2=SCY(0x50000);		// yvel range
	(long)current_proc->a3=SCY(0x30000);		// yvel min
...
******************************************************************************/
short face_blood_adjustments[][2]=
{
	{SCX(0x0008),SCY(0x0010)},
	{SCX(0x0008),SCY(0x0010)},
	{SCX(0x000c),SCY(0x0015)},
	{SCX(0x0008),SCY(0x0010)},
...
void face_punch_blood(void)
{
	...
	multi_adjust_xy(current_proc->pa8,face_blood_adjustments[current_proc->a0][0],face_blood_adjustments[current_proc->a0][1]);
...
void spawnd3(void)
{
	...
	ptemp=CREATE(PID_BLOOD,(FUNC)current_proc->pa7);
...
void gbld3(void *pa5)
{
	...
	if (ta3->ochar<=FT_SK)
		(WORD *)current_proc->pa0=ocharBLOOD_Palettes[ta3->ochar];
	else (WORD *)current_proc->pa0=ocharBLOOD_Palettes[0];
