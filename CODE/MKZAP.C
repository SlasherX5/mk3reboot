...
void motaro_zap_proc(void)
{
	...
	(FUNC)current_proc->pa6=mot_zap_call;
...
/******************************************************************************
 Function: void do_proj_smoke_spear(void)
...
******************************************************************************/
int kludge_death_count = 0;
int kludge_smoke_spear = 0;
int kludge_obj;
int kludge_obj_pos;
void do_proj_smoke_spear(void)
{
	...
	get_char_ani(ANIM_TABLE2,ANIM_ROBO_SPEAR);
	get_proj_obj_m(obj);

//WIN95: VERY CHEEZY HACK TO GET SMOKE SPEAR TO WORK!!!
kludge_obj_pos = ((OBJECT *)current_proc->a10)->oxpos.u.intpos;
kludge_obj = (OBJECT *)current_proc->a10;

	adjust_xy_a5((OBJECT *)current_proc->a10,-SCX(0x120),SCY(0x28));		// adjust to smoke's chest

	insert_object((OBJECT *)current_proc->a10,&objlst);

	pa0=create_proj_proc(spear_proc);
	pa0->pdata.p_store5=(ADDRESS)current_proc;		// pass smoke proc
	pa0->pdata.p_store6=(ADDRESS)current_proc->pa8;	// pass smoke obj

	current_proc->p_comp_flag=PC_WAIT_SPEAR2;

	process_sleep(0x18);

	/* waiting_for_spear2 */
	current_proc->p_comp_flag=PC_WAIT_SPEAR;
	current_proc->pdata.p_action=ACT_PROJ_SD;
	process_sleep(0x18);

	/* waiting_for_spear */
	current_proc->p_comp_flag=PC_CLEAR;
	current_proc->pdata.p_action=0;			// i am doing nada
...
void spear_proc(void)
{
	...
	current_proc->a0=current_proc->a11=0x15;
	tell_world_stk(0x15);

//WIN95: VERY CHEEZY HACK TO GET SMOKE SPEAR TO WORK!!!
kludge_smoke_spear = 1;
kludge_death_count = 32;
	...
	do
	{
		...
		{
//WIN95: VERY CHEEZY HACK TO GET SMOKE SPEAR TO WORK!!!
kludge_death_count = 0;
kludge_smoke_spear = 0;
kludge_obj = 0;

			delete_proj_and_die(obj);
		}

		tell_world_stk(current_proc->a11);
	}
	while (proj_strike_check(obj,current_proc->a11)==SYSTEM_CARRY_CLR);
...
kludge_obj = 0;
	...
	stop_a8(obj);
	(ADDRESS)pa0=current_proc->pdata.p_store5;		// dude who threw spear

	if (pa0->p_comp_flag!=PC_WAIT_SPEAR2 && pa0->p_comp_flag!=PC_WAIT_SPEAR)
	{
		/* spear_cancelled */
	...
	fastxfer(pa0,do_rope_pull);
	pa0->a10=(ADDRESS)current_proc->pa8;		// pass a10 - spear proc
	pa0->pdata.p_slave=current_proc->pa8;		// rope = your responsibility
...
void do_rope_pull(void)
{
	...
	current_proc->pdata.p_store8=((OBJECT *)current_proc->a10)->header.t_xoffset;		// base offset in tpage for rope
	...
	((OBJECT*)current_proc->a11)->ozval=BACK_Z-1;
	((OBJECT*)current_proc->a10)->ozval=BACK_Z;
	/* proj_in_front */
	obj->ozval=FRONT_Z;
	(current_proc->pdata.p_otherproc)->pdata.p_action=ACT_SPEARED_SD;		// define otherdudes action
...
void lineup_rope_with_him(void)
{
	...
	current_proc->pa8=(OBJECT *)current_proc->a10;
	current_proc->a7=(current_proc->pa8)->oypos.u.intpos;
	match_me_with_him();
	(current_proc->pa8)->oypos.u.intpos=current_proc->a7;
	flip_multi(current_proc->pa8);
	multi_adjust_xy(current_proc->pa8,-SCX(0x130),0);
	current_proc->pa8=pa8;

	//set_rope_clip(current_proc->a10);
	...
	//	(short)current_proc->a0=-(short)current_proc->a0;
...
void do_proj_sk_zap(void)
{
	...
	(OBJECT *)current_proc->pa10=get_proj_obj_m(current_proc->pa8);
	insert_object((OBJECT *)current_proc->a10,&objlst);
...
void do_proj_tusk_floor(void)
{
	...
	ta9=current_proc->pa9;
	(OBJECT *)current_proc->pa10=ta10=current_proc->pdata.p_slave;
	current_proc->pdata.p_slave=NULL;
	get_char_ani(ANIM_TABLE2,ANIM_TUSK_BLADE);
	get_proj_obj_m(current_proc->pa8);
	create_proj_proc(blade_proc);
	current_proc->pa9=ta9;
	(OBJECT *)current_proc->pa10=ta10;
...
void bomb3(FUNC pa11)
{
	...
	/* bomb4 */
	if (current_proc->a3<2)			// only allow 2 bombs
	{
		ta9=current_proc->pa9;
		get_char_ani(ANIM_TABLE2,ANIM_ROBO_BOMB);
		current_proc->a11=(ADDRESS)current_proc->pa9;
		(OBJECT *)current_proc->pa10=get_proj_obj_m(obj);
		insert_object((OBJECT *)current_proc->a10,&objlst);
		current_proc->pa9=ta9;
...
void rocket1_proc(void)
{
	...
	get_char_ani(ANIM_TABLE1,ANIM_ROCKET);
	...
	(FUNC)current_proc->pa6=rocket1_flight_call;
...
void rocket2_proc(void)
{
	...
	current_proc->pdata.p_store1=(ADDRESS)rocket_routines[0].r_accelcnt;
	...
	((OBJECT *)current_proc->pdata.v_store6)=objtarget;		// target obj
	((void *)current_proc->pdata.v_store7)=current_proc->pa9;		// target ani
...
void do_proj_lia_anglez(void)
{
	...
	(OBJECT *)current_proc->pa11=current_proc->pdata.p_slave;
...
void lao_hat_proc(void)
{
	...
	(FUNC)current_proc->pa6=lao_zap_call;
...
void do_proj_jax_zap2(void)
{
	...
	process_sleep(0x1d);				// even LONGER DELAY
...
{
	if (is_he_motaro()==SYSTEM_CARRY_SET)
...
OBJECT *get_proj_obj_m(OBJECT *obj)
{
	...
	objslave=(OBJECT *)current_proc->pa10=current_proc->pa8;		// a10 = object
