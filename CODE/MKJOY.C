...
void switcheroo_wake(void)
{
	...
	WORD new_char;
	/* switch to other guy */
	while ((new_char=randu(14)-1)==(current_proc->pa8)->ochar);			// dont pick same dude
...
//	goto *jumptbl[jumpindex];               ****************** MAYBE NEEDS_PORTING
...
JUR4:
	...
	(ADDRESS *)current_proc->pa6=(ADDRESS *)(angle_jump_call);
...
void angle_jump_call(void)
{
	...
	if (current_proc->pa8->oyvel.pos>0)				// vertical direction, upwards skip
	{
		...
		if (abs(current_proc->pdata.p_ganiy-current_proc->pa8->oypos.u.intpos)<20)
		{
			...
			(ADDRESS *)current_proc->pdata.v_store4=NULL;	// no need for this code any more
		}
	}
...
WORD bar_reducer(WORD ptype,WORD *damage)
{
	...
	if (ta1->oflags2 & M_HALF_DAMAGE)		// half damage protection
		(*damage)>>=1;							// yes, slice it
...
void turbo_bar_setup(void)
{
	if (p1_obj==current_proc->pa8)
	{
		(WORD *)current_proc->pa5=&p1_turbo;
		(WORD *)current_proc->pa6=&p1_dont_raise;
	}
	else
	{
		(WORD *)current_proc->pa5=&p2_turbo;
		(WORD *)current_proc->pa6=&p2_dont_raise;
	}
}
...
void ochar_begin_calls(void)
{
	switch ((current_proc->pa8)->ochar)
	{
		case FT_SMOKE:
			...
			if (f_smoke==0)
				diff=9;						// smoke still hidden=tough
			break;
		case FT_NOOB:
			diff=8;					// noob is a touch lil cookie
			break;
	}
