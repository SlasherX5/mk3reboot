...
extern int  Wav95_Play(int id,int loop);
extern void Wav95_Stop(int id);
...
#define MINIMUM_TRACK_INDXS_FOR_A_SEQUENCE 10
...
static Wess_File_IO_Struct *fp_wmd_file = 0;
...
static patch_group_header scratch_pat_grp_hdr;
static track_header scratch_trk_hdr;

static int wmd_mem_is_mine = 0;
static char *wmd_mem = 0;
static char *wmd_end = 0;
static int wmd_size = 0;

static int (*Error_func)(int, int) = 0;
static int Error_module = 0;
...
static void zeroset(char *pdest, unsigned long size)
{
    while(size--) *pdest++ = 0;
}
#endif

void wess_install_error_handler(int (*error_func)(int, int), int module) {
#ifndef BLOCK_SOUND_WESS	
...
void wess_install_handler (void)
{
#ifndef BLOCK_SOUND_WESS	
    init_WessTimer();
#endif
...
void wess_restore_handler (void)
{
#ifndef BLOCK_SOUND_WESS	
    exit_WessTimer();
#endif
...
void wess_exit (enum RestoreFlag rflag)
{
#ifndef BLOCK_SOUND_WESS	
...
static void free_mem_if_mine(void)
{
#ifndef BLOCK_SOUND_WESS	
    if(wmd_mem_is_mine)
    {
...
void wess_unload_module (void)
{
    ...
    if(module_loaded)
    {
        ...
        for(mi=0;mi<ptif;mi++)
        {
            if((pm_stat->ppat_info+mi)->hw_tl_list.sfxload ||
               (pm_stat->ppat_info+mi)->hw_tl_list.musload ||
               (pm_stat->ppat_info+mi)->hw_tl_list.drmload)
            {
...
int conditional_read(int    readflag,
                     char **ptrtomem,
                     int    readsize)
{
#ifndef BLOCK_SOUND_WESS	
    ...
    if(readflag)
    {
        readrequest = readsize;
        readresult = module_read(*ptrtomem,readrequest,fp_wmd_file);
                      ...
                      int **settings_tag_lists)
{
    ...
    {
        wmd_mem_is_mine = 1;
        if((wmd_mem = (char *)malloc(memory_allowance))==NULL)
        {
    ...
    */
    ...
    pm_stat->fp_module = (int)fp_wmd_file;
...
void filltrackstat(void *ptk_stat, void *ptk_info, void *attr)
{
#ifndef BLOCK_SOUND_WESS	
    ...
    ptk_stat->channel = ptk_info->trk_hdr.lockchannel;
...
void assigntrackstat(void *ptk_stat, void *ptk_info)
{
#ifndef BLOCK_SOUND_WESS	
...
int wess_seq_structrig (void *psq_info,
                        int seq_num,
                        int seq_type,
...
//    _disable();
    ...
    for(i=0;i<limit;i++)
    {
        if(!(pm_stat->pseqstattbl+i)->active)
        {
            break;
        }
    }
...
//        _enable();
        ...
        */
        ...
        for(j=0;j<limit;j++)
        {
            if(!(pm_stat->ptrkstattbl+j)->active)
            {
...
void wess_seq_trigger_type (int seq_num, int seq_type)
{
#ifndef BLOCK_SOUND_WESS	
...
void wess_seq_trigger (int seq_num)
{
#ifndef BLOCK_SOUND_WESS	

    wess_seq_trigger_type (seq_num, 0);
#else
    Wav95_Play(seq_num,0);
#endif
}
...
int wess_seq_status (int sequence_number)
{
#ifndef BLOCK_SOUND_WESS	
...
void wess_seq_stop (int sequence_number)
{
#ifndef BLOCK_SOUND_WESS	
    ...
    SeqOn = 1;
#else
    Wav95_Stop(sequence_number);
#endif
}

void wess_seq_stoploop (int sequence_number)
{
#ifndef BLOCK_SOUND_WESS	
...
void wess_seq_stoptype (int type_number)
{
#ifndef BLOCK_SOUND_WESS	
...
void trackstart(void *ptmp, void *psq_stat)
{
#ifndef BLOCK_SOUND_WESS	
...
void trackstop(void *ptmp, void *psq_stat)
{
#ifndef BLOCK_SOUND_WESS	
...
void wess_seq_pause (int sequence_number,enum MuteFlag mflag)
{
#ifndef BLOCK_SOUND_WESS	
...
void wess_seq_restart (int sequence_number)
{
#ifndef BLOCK_SOUND_WESS	
...
#else
    Wav95_Play(sequence_number,0);
#endif
}

void wess_seq_pauseall (enum MuteFlag mflag)
{
#ifndef BLOCK_SOUND_WESS	
...
void wess_seq_restartall (void)
{
    ...
    /*------------------------------------------------------------------*/
    /*
        The handle oriented sound calls are designed for fine control
        over sounds and sequences.

      - handles may do the following:
        reads sequencer status to see if this sequence is playing
        if the sequence in not playing we attempt to allocate channels
        needed by this sequence
        if the sequence is playing we check for channels allocated under
        a sequence number, allocate what is needed and flag all with handle
        flags these channels for being in use by the returned handle
    */
    /*------------------------------------------------------------------*/

    /*
        This section of handle calls can be called on a running
        sequence handle or a stopped sequence handle.
        To work correctly, these functions shutoff the seq_engine
        during operation and fake the ppos data.
    */

void *gethandleseq(int handle)
{
#ifndef BLOCK_SOUND_WESS	
    ...
    if(handle>0&&handle<=pm_stat->pmod_info->mod_hdr.seq_work_areas)
    {
        hindx = handle - 1;
        if((psq = (pm_stat->pseqstattbl+hindx))->handle)
        {
            return(psq);
        }
    }
...
void *gethandletrk(int handle, int track)
{
#ifndef BLOCK_SOUND_WESS	
    ...
    if(handle>0&&handle<=pm_stat->pmod_info->mod_hdr.seq_work_areas)
    {
        ...
        if((psq = (pm_stat->pseqstattbl+hindx))->handle)
        {
            ...
            {
                if((ptrk = pm_stat->ptrkstattbl+*(psq->ptrk_indxs+track))->handled)
                {
                    return(ptrk);
                }
            }
...
void wess_register_callback (char marker_ID, void (*function_pointer)(char,short))
{
#ifndef BLOCK_SOUND_WESS	

    static char si,sn;
    static callback_status *cbs;
    ...
    si = pm_stat->pmod_info->mod_hdr.callback_areas;
...
void wess_delete_callback (char marker_ID)
{
#ifndef BLOCK_SOUND_WESS	
...
int wess_master_vol_get (int sound_driver)
{
#ifndef BLOCK_SOUND_WESS	
    ...
    /*
        Specify sound_driver with the PatchIds enum!
    */

    if(sound_driver>0&&sound_driver<pm_stat->patch_types_loaded)
    {
        i = pm_stat->pmaster_volume[sound_driver-1];
    } else {
        i = 0;
    }
...
void wess_master_vol_set (int sound_driver, int volume_parm)
{
#ifndef BLOCK_SOUND_WESS	
    ...
    /*
        Well, the fun thing about master volume is that all notes
        playing on all tracks of all active sequences must be modified
        in volume! Oh boy!
        Specify sound_driver with the PatchIds enum!
    */
    ...
    if(sound_driver>0&&sound_driver<pm_stat->patch_types_loaded)
    {
        pm_stat->pmaster_volume[sound_driver-1] = volume_parm;
    } else {
        pm_stat->pmaster_volume[sound_driver-1] = 0;
    }

    /* process all affected volumes */
#endif
}

void * wess_get_master_status (void)
{
#ifndef BLOCK_SOUND_WESS	
