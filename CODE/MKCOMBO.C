...
void do_elbow(void)
{
	...
	if (striker(SI_STRIKER_NORMAL,1,ACT_ELBOW,ANIM_ELBOW,5,0xf)==SYSTEM_CARRY_SET)
	{
...
COMB0:	/* comb0 */
	...
	else
	{
		...
		do
		{
			...
			if (combo_scan_a11(&ctbl)==SYSTEM_CARRY_SET)
				goto COMB8;
		}
...
COMB8:
	...
	if (strike_check(combo_strike_table[(ctbl->c_stk)>>8],0)==SYSTEM_CARRY_SET)
	{
		...
		(COMBO *)current_proc->pa7=ctbl;

#if SONY_PSX
		/* -SONY- conv note code spcae is 0x8000000 or greater */
#endif
//WIN95	if ((LONG)ctbl->c_next >= 0x80000000)
		if ((LONG)ctbl->c_next >= 0x400000)
		{
...
{
	find_ani_part_a14(pa9>>8,pa9 & 0xff);
...
{
	(PROCESS *)current_proc->pdata.v_store4=current_proc;		// this is assumed
