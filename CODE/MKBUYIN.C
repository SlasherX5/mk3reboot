...
extern char txt_1_coin[];
extern char txt_2_coins[];
extern char txt_x_coins[];
extern char txt_press_start[];
extern char txt_insert_coin[];
...
void do_buyin(void)
{
	...
	if (p2_state!=PS_ACTIVE)
	{
		...
		adjust_menu_down(&objlst2);
	  move_object_list(&objlst2,&objlst);
	}

	//if ( f_freeplay )
	//				p7_centered(txt_freeplay,84,0xe7-2);
	//		else lm_printf_p1(pf_credit_info,credits);
	//adjust_menu_down(&objlst2);
	//move_object_list(&objlst2,&objlst);

	/* buy2 */
	ptemp=CREATE(PID_FX,boonpal_stuff);
	(void *)ptemp->pa11=fl_num;
	...
	*/
	...
	play_generic_tune(TRK_OLD);				 // send_code_a3(0x38);			// buyin tune
...
******************************************************************************/
 void elim2(WORD *pstate,WORD *pwiar)
{
	if (*pstate==PS_BUYIN)
	{
...
#if 0
/******************************************************************************
 Function: char *get_ctc_msg(void)
 ...
 Description:	get correct credit message to display
******************************************************************************/
