...
void zoom_blocked(OBJECT *pa8)
{
	...
	match_me_with_him();
	flip_multi(pa8);
	multi_adjust_xy(pa8,-SCX(8),SCY(0));
...
void prop_do_lao_angle(void)
{
	...
	(FUNC)current_proc->pa6=lao_angle_scan;
...
void cyrax_implode(void)
{
	...
	do
	{
		...
		lineup_1pwm(obj,(OBJECT *)current_proc->a10);
		insert_object(obj,&objlst);
		(PROCESS *)current_proc->pa0=CREATE(PID_ROBO_LIMB,robo_limb_in);
		obj=current_proc->pa8=(OBJECT *)current_proc->a10;
		current_proc->a11++;
	}
	while(current_proc->a11!=9);
...
void prop_do_robo_tele(void)
{
	...
	obj->oyvel.pos=SCY(0x68000);			// start headin' downward

	obj->ograv.pos=SCY(0xa000);
	...
	multi_adjust_xy(obj,-SCX(0x40),0);		// line me up to hit him !!
	...
	(FUNC)current_proc->pa6=tele_scan2;
...
void prop_kano_cannon_ball(void)
{
	...
	get_char_ani(ANIM_TABLE1,ANIM_FFLIP);
	GET_LONG(current_proc->pa9)++;							// ball roll ani
...
void prop_sonya_bike_kick(void)
{
	...
	(ADDRESS *)current_proc->pa6=(ADDRESS *)(bike_scan_call);
...
void prop_do_sz_decoy(void)
{
	...
	get_char_ani(ANIM_TABLE1,ANIM_BFLIP);
	GET_LONG(current_proc->pa9)++;			// backward flip ani
...
void decoy_proc(void)
{
	...
	(OBJECT *)current_proc->pa11=current_proc->pa8;		// a11 = orig subzero
	ptemp=gmo_proc((current_proc->pa8)->oheap);									// a8 = decoy object
	obj_decoy=current_proc->pa8;
	(PROCESS *)(current_proc->pdata.v_store3)=ptemp;
	...
	loop=(is_he_joy()==SYSTEM_CARRY_SET)?0x40:0x2a;		// vs joy==longer, vs drone == shorter
	/* decoy3 */
	do
	{
		process_sleep(1);
...
void prop_do_slide(void)
{
	...
	towards_x_vel(current_proc,obj,SCX(0x80000));		// head towards opponent
