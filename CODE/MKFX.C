...
/* animality */
#if 0
extern void *ANIMALITY01[];
extern void *a_animality[];
extern void *animal_anims[];
#endif

/* fatality */
#if 0
extern void *FATALITY01[];
extern void *a_fatality_drip[];
extern void *fatality_anims[];
#endif

/* baby stuff */
#if 0
extern void *baby_anims[];
extern void *BABALITY1[];
extern void *BABALITY2[];
extern void *BABALITY3[];
extern void *BABALITY4[];
extern void *BABALITY5[];
extern void *BABALITY6[];
extern void *BABALITY7[];
extern void *BABALITY8[];
#endif

/* friendship stuff */
#if 0
extern void *friend_anims[];
...
{
	playsnd(TUNE_BABAL1);	// SCA3(0x43) baby music
	process_sleep(4);
	...
	CREATE(PID_FX,baby_letter);
	process_sleep(0x60);
	playsnd(TS_SK_BABALITY);				// speech babality
...
#if 0
	baby_text=(ADDRESS*)get_char_ani(ANIM_TABLE2,ANIM_BABY_LETTER);			// get ptr to baby_text table
	(ADDRESS)current_proc->pa9=*(baby_text+current_proc->a11);				// get ptr to correct letter
	current_proc->pa9=(void *)COMPUTE_ADDR(obj->oheap,current_proc->pa9);
	gso_dmawnz_ns(obj,(ADDRESS)current_proc->pa9,obj->oheap,0);
...
{
	playsnd(TS_SK_FRIEND);			// friendship
	process_sleep(0x40);
	playsnd(TS_SK_FRIEND1);			// friendship
	//process_sleep(0x40);
...
//	play_generic_stream(triple_sndtab[current_proc->a10+0x4e]);			// tsound(0x5d)
	tsnd0(triple_sndtab + (current_proc->a10+0x4e),64);
...
//***********************************
	if (oa11->ochar<=FT_SK)
		(WORD *)current_proc->pa0=ochar_bloodxpd_palettes[oa11->ochar];
	else (WORD *)current_proc->pa0=ochar_bloodxpd_palettes[0];

	if ((WORD *)current_proc->pa0!=BLDXPD_P)
...
//***********************************
	if (oa11->ochar<=FT_SK)
		(WORD *)current_proc->pa0=ochar_blooddrip_palettes[oa11->ochar];
	else (WORD *)current_proc->pa0=ochar_blooddrip_palettes[0];

	if ((WORD *)current_proc->pa0!=SMDRIP_P)
		swpal((void *)current_proc->pa0,obj);			// not normal blood switch
//***********************************
...
void smoke_smoke(void)
{
	...
	do
	{
		...
		while((current_proc->pa8)->ochar!=FT_SMOKE || (current_proc->pa8)->oflags2 & M_INVISO);
	}
	while(1);
}
...
void setup_combo_message(short pa4x,short pa4y,void *pa5,WORD pa6,WORD pa11)
{
	...
	if (ta3!=0)
	{
		(OBJECT*)current_proc->pa10=a3_combo_number_obj(ta3,pa4x,pa4y,pa5);		// a10 = 1st digit
	}
...
void fx_inviso_poof(void)
{
	...
	multi_adjust_xy(obj,SCX(0),SCY(0x90));
	insert_object(obj,&objlst);
...
void robo_bomb_explode(void)
{
	...
	(OBJECT *)current_proc->pa10=current_proc->pa8;
...
OBJECT *puff_init(WORD zval)
{
	...
	(OBJECT *)current_proc->pa10=current_proc->pa8;			// a10 = rocket to follow
...
void trail_proc(void)
{
	...
	(OBJECT *)current_proc->pa11=oa11=current_proc->pa8;
	...
	(OBJECT *)current_proc->pdata.v_store2=obj_free;	// save first object
...
//WIN95: Fix a bad use of palette ram call 
		get_multi_obj((ADDRESS *)current_proc->a5,&(ptemp->pdata),oa11->oheap,PFLAG_PALLOAD);
//		get_multi_obj((ADDRESS *)current_proc->a5,NULL,oa11->oheap,PFLAG_PALLOAD);
	...
	do
	{
		(OBJECT *)current_proc->pa10=oa11->oplink;
		delobjp(oa11);
		process_sleep(2);
	}
...
//WIN95: FIX_HARD_SONY_MEMORY_ADDRESSES
//	if ((long)current_proc->pa9<0)
//		get_char_ani(ANIM_TABLE1,(LONG)current_proc->pa9);
//WIN95	if ((long)current_proc->pa9<0x8fff)
	if ((long)current_proc->pa9<0x400000)
		get_char_ani(ANIM_TABLE1,(LONG)current_proc->pa9);
...
void freeze_blast(void)
{
	...
	(OBJECT *)current_proc->pa11=obj;		// a11 = decoy obj
