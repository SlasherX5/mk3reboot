...
******************************************************************************/
PROCESS *process_create0(WORD procid,void (*func)(void),const char* name)
{
	...
	if(free_head != NULL)
	{
		...
		new_proc->plink=current_proc->plink; // link new process ahead of current process

		current_proc->plink = new_proc;	// link current process to new process

		// initialize context from current process
		new_proc->p_context = current_proc->p_context;

		new_proc->NAME = name;

		InitProcessStack(new_proc);
		new_proc->p_context.c_wakeup=func;		// set wakeup point

		new_proc->ptime=1;	// set sleep time to start process right away 
		new_proc->procid=procid;	// set id 
...
extern void Win95_OUT(char*buff);
extern int pheap_beginmark;
extern int pheap_endmark;
...
void process_dispatch()
{
	...
	int loop;
	for (loop=0;loop<NUM_PROCESS-1;loop++)
		if (process_ram[loop].endmark != -1) {
			Win95_OUT("Stack overflow\n\r");
		}			

	if ( pheap_beginmark != 0xFFFFFFFF )
		Win95_OUT("Player heap corubted\n\r");

	if ( pheap_endmark != 0xFFFFFFFF )
		Win95_OUT("Player heap corubted\n\r");
	}
#endif
