...
void d_attack_very_far(void)
{
	...
	{
		stack_jump(d_zap);			// very_far_airborn
	}
}
...
{
	stance_wait_yes(randu_minimum(0x40,0x16),q_drone_zone);
...
{
	frontflip_setup();
	(FUNC)current_proc->pa6=fflip_scan;
	d_fflip_scan_jsrp();
	d_reaction_exit();
}
...
void run_in_close_now(void)
{
	...
	if (is_he_blocking(&dflag,0)==SYSTEM_CARRY_CLR)
	{
...
void d_fflip_jsrp(void)
{
	...
	{
		(FUNC)current_proc->pa6=fflip_watchout;
		d_fflip_scan_jsrp();
	}
	return;
}
...
void d_backup_jsrp(void)
{
	...
	current_proc->a10=(ADDRESS)current_proc->a9;
	((ADDRESS *)current_proc->pa9)+=2;
	d_do_next_a9_frame(current_proc->pa8);
	process_sleep(2);
	d_beware();

	(ADDRESS)current_proc->a9=current_proc->a10;
...
/******************************************************************************
 Function:  void nr_hikick_if_u_can(void)
...
/******************************************************************************
 Function:  void nr_sweep_if_u_can(void)
...
******************************************************************************/
 void nr_sweep_if_u_can(void)
{
	if_u_can(4,d_sweep_kick);
}

/******************************************************************************
 Function:  void nr_uppercut_if_u_can(void)
...
******************************************************************************/
 void nr_uppercut_if_u_can(void)
{
...
void d_stance_setup(void)
{
	...
	current_proc->a5=(ADDRESS)current_proc->a9;
	stance_setup();
	...
	/* sss2 */
	current_proc->a4=(ADDRESS)current_proc->a9;

	/* sss3 */
	while ((GET_LONG(current_proc->pa4)++)!=ANI_JUMP);
	((ADDRESS *)current_proc->pa4)-=2;			// a4-> last stance frame

	/* a9 --> top 	a4 --> bottom	a5--> where we were before */
	if (current_proc->a5<(ADDRESS)current_proc->a9)
		return;					//	b4 -> above the top --> use new ani

	if (current_proc->a5>current_proc->a4)
		return;					//	b4 -> below the bottom --> use new ani

	(ADDRESS)current_proc->a9=current_proc->a5;
...
/******************************************************************************
 Function:  void drone_long_jump(WORD pa0)
...
extern short ochar_fatality_distances[][2];

void d_finish_him(void)
{
	boss_branch(0x11);

#ifdef FORCE_DRONE_FATALITY
	...
	if (f_no_violence==0 && f_no_blood==0)
	{
		...
		{
			if (curback==BKGD_SUBWAY_MOD || curback==BKGD_PIT_MOD || curback==BKGD_BELL_MOD )
			{
...
JUMPTBL **ochar_cat_tables[]=
{
	...
	motaro_catagory_table,	// 15 - motaro
	sk_catagory_table,		// 16 - shao kahn
...
void c_tusk_blur_sd(void)
{
	...
	if (randper(500) == SYSTEM_CARRY_CLR)
		d_elbow();
	else {
		do_duck_punch();
		do_duck_punch();
...
{
	if (((PROCESS *)current_proc->pa0=process_exist(PID_BLADE,-1))==NULL)
		q_no;
	else q_yes;
}
...
void wait_proj_pass(void)
{
	...
	do
	{
		...
		(WORD *)current_proc->pa0=ta0->pdata.p_joyport;		// a0 = stk table
...
void react_jump_table(WORD pa1,FUNC *pa2)
{
	...
	{
		if (randper(d_randpers[diff/2])==SYSTEM_CARRY_CLR)
		{
			return_to_beware;
		}
		else
