...
void group_sound(WORD offset)
{
	...
	{
//		if ( current_proc->a2*current_proc->a1 > 10 || current_proc->a2*current_proc->a1 < 0 )	
//			stbl+=(current_proc->a2*current_proc->a1);	// # of words to skip to get to our group data
//		else			
			stbl+=((short)current_proc->a2*(short)current_proc->a1);	// # of words to skip to get to our group data
	}
...
short triple_sndtab[]=
{
	...
	WAV_TRK_SK_FIGHT,	//TRK_FIGHT		// 10 - Shao Kahn: Fight! #3
	...
	WAV_TRK_SHEEVA_WINS,
	WAV_TRK_SHANG_WINS,
	WAV_TRK_LIUKANG_WINS,
	WAV_TRK_SMOKE_WINS,
	WAV_TRK_MOTARO_WINS,
	TRK_SK_WINS,						// 4d - Shao Kahn: I wins

	TS_KANO_MERCY,											// 4e - Shao Kahn: Kano shows mercy
	TS_SONYA_MERCY,											// 4f - Shao Kahn: Sonya shows mercy
	TS_JAX_MERCY,											// 50 - Shao Kahn: Jax shows mercy
	TS_INDIAN_MERCY,										// 51 - Shao Kahn: Indian shows mercy
	TS_SUBZERO_MERCY,										// 52 - Shao Kahn: Sub-Zero shows mercy
	TS_SWAT_MERCY,											// 53 - Shao Kahn: Stryker shows mercy
	TS_LIA_MERCY,											// 54 - Shao Kahn: Sindel shows mercy
	TS_ROBO1_MERCY,											// 55 - Shao Kahn: Sektor shows mercy
	TS_ROBO2_MERCY,											// 56 - Shao Kahn: Cyrax shows mercy
	TS_LAO_MERCY,											// 57 - Shao Kahn: Kung Lao shows mercy
	TS_TUSK_MERCY,											// 58 - Shao Kahn: Cabal shows mercy
	TS_SHEEVA_MERCY,										// 59 - Shao Kahn: Sheeva shows mercy
	TS_SHANG_MERCY,											// 5a - Shao Kahn: Shang Tsung shows mercy
	TS_LIUKANG_MERCY,										// 5b - Shao Kahn: Liu Kang shows mercy
	TS_SMOKE_MERCY,											// 5c - Shao Kahn: Smoke shows mercy
...
// place holders for sounds on Red Book These sounds don't exsit 
...
extern void Win95_ResetSpeed();

static int _music_playing = 0;
...
void psxspu_start_cd_fade(int msec, int destvol)
{
	stop_tunes();

	_music_playing = 0;

	PROCESS* mp = process_exist(PID_MUSICPLAYER, 0xff);
...
{	
	...
	while ((id= ((WORD*)current_proc->pa0)[current_proc->a1++]) != 0xffff) {
		if (id < 10) {
			current_proc->a1 = id;
			continue;
		} else if (id < NUMBER_OF_SEQUENCES) {
			int duration = Wav95_Play(id,0);
			if (duration) {
				_music_playing = 1;
				long timeEnd = Win95_timeGetTime() + duration;
				do
				{
					process_sleep(1);
				} while (Win95_timeGetTime() < timeEnd && _music_playing == 1);
		...
		if (!_music_playing) break;		
	}

	process_suicide();
}

void play_generic_tune(WORD* trkdata)
{
	...
	PROCESS* mp = process_exist(PID_MUSICPLAYER, 0xff);
	if (mp == NULL) {
		mp = process_create(PID_MUSICPLAYER, play_the_music);		
	} else {
		mp->ptime = 1;
		stop_tunes();
	}
	mp->pa0 = trkdata;
	mp->a1 = 0;
	_music_playing = 2;//music changed
	Win95_ResetSpeed();
}
...
short st_robo1[]=
{
	...
	FT_ROBO_CRSH_SMOOSH2,			//19 - crusher smoosh
...
void sound_module_load(WORD mod)
{
	...
	if (snd_module_status != mod)
	{
		...
		if (mod==SMOD_FIGHT)
			...
			PsxSoundLoadData(module_table[mod-1]);
...
WORD sound_pan_value(void)
{
	...
	return((WORD)32+((128*pos) / (SCRRGT/2) ) /4);	
	/*	  pan value
	32---------64---------96
	left     center		 right
	*/
