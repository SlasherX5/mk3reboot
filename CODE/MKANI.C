...
u_short DLoadTPageFighter(u_short* pix, int x, int y, int w, int h, int pal, int fighter, int fver, int fo, BMPINFO** info);
...
void animate2(OBJECT *obj,ADDRESS *framelist)
{
	...
	{
#else
	LONG frame_offset = GET_LONG(framelist);
#endif

		frame=(OIMGTBL *)COMPUTE_ADDR(obj->oheap, frame_offset);
...
*************************************************/
			...
			obj->header.tpage= DLoadTPageFighter(
				(u_short*)pram_ptr->prsagptr,
				frame_info.x,frame_info.y,
				(pram_ptr->prsize.u.xpos+3)&(~3),frame_info.h,
				obj->opal,f, obj->over, frame_offset, &obj->bmp);

			obj->header.t_xoffset=(frame_info.x%128)*2;
			obj->header.t_yoffset=frame_info.y%256;
			obj->header.t_width = obj->bmp ? SCX(obj->bmp[0]) : frame_info.w;
			obj->header.t_height= obj->bmp ? SCY(obj->bmp[1]) : frame_info.h;

			if (((pram_ptr->prflags & 0x30) >> 4) == 1 && obj->bmp) {
				pram_ptr->prpos.u.xpos += obj->header.t_width - frame_info.w;
			}
...
******************************************************************************/
 void do_first_a9_frame(short animnum)
{
	get_char_ani(ANIM_TABLE1,animnum);
	...
	while (animframe != ANI_END)
	{
		switch (animframe)
		{
			...
			case ANI_ADJUSTXY:	// animation script "adjust xy" command
				...
				if (!editmode) multi_adjust_xy(obj,offset,offset1);
				break;
			...
			case ANI_OCHAR_JUMP:	// animation script ochar conditional jump
				...
				if (animframe==obj->ochar)
					(ADDRESS *)current_proc->pa9=(ADDRESS *)COMPUTE_ADDR(obj->oheap,GET_LONG(current_proc->pa9));	// get new animation ptr
				else GET_LONG(current_proc->pa9)++;				// skip past jump
				break;

			case ANI_FLIP_V:		// animation script vertical flip
				if (!editmode) flip_multi_v(obj);
				break;
			...
			case ANI_OFFSET_XY:		// animation script offset xy
				...
				if (obj->oflags & M_FLIPH)
					offset=-offset;
				if (!editmode) obj->oxpos.u.intpos=offset+(current_proc->pdata.p_otherguy)->oxpos.u.intpos;

				offset=(short)(GET_WORD(current_proc->pa9)++);
				offset=offset*85/100;
				if (!editmode) obj->oypos.u.intpos=offset+(current_proc->pdata.p_otherguy)->oypos.u.intpos;
				break;
			...
			case ANI_OCHAR_SOUND:
				...
				break;
			case ANI_RND_JUMP:				
				if (p2_matchw > 1 || p1_matchw > 1 
					|| randper(250) == SYSTEM_CARRY_SET
					)
				{
					(ADDRESS*)current_proc->pa9 = (ADDRESS*)COMPUTE_ADDR(obj->oheap, GET_LONG(current_proc->pa9));	// get new animation ptr
				}
...
void mframew_him(WORD sleeptime)
{
	...
	animtemp=(ADDRESS)current_proc->a9;
	(ADDRESS)current_proc->a9=current_proc->a11;
	current_proc->pa8=current_proc->pdata.p_otherguy;
	mframew(sleeptime);
	current_proc->pa8=obj;
	current_proc->a11=(ADDRESS)current_proc->a9;
	current_proc->a9=animtemp;
...
{
	(ADDRESS *)current_proc->pa10=(ADDRESS *)current_proc->pa9;
...
WORD frame_a9(OBJECT *obj)
{
	...
	if (animframe==ANI_SWPAL)
	{
		...
		OIMGTBL* panimframe=(OIMGTBL*)COMPUTE_ADDR(obj->oheap,animframe);
		panimframe++;
		swpal((void*)(GET_LONG(panimframe)), obj);
...
WORD double_next_a9(void)
{
	...
	ptemp=current_proc;
	current_proc=current_proc->pdata.p_otherproc;
	animframe=current_proc->a9;

	(ADDRESS)current_proc->a9=animtemp;
	results=do_next_a9_frame((OBJECT *)ptemp->a10);
	animtemp=current_proc->a9;

	/* restore stuff */
	(ADDRESS)current_proc->a9=animframe;
	current_proc=ptemp;
