 ...
 * MODULE PROTOTYPES
 */
void stack_switch_bits(LONG,WORD);
...
/*this line sets the hardware to pal mode */
	...
	db[0].clear.x=BUF0_X_COORD;
	db[0].clear.y=BUF0_Y_COORD;
	...
	db[1].clear.x=BUF1_X_COORD;
	db[1].clear.y=BUF1_Y_COORD;
...
void back_unbetterSimpleRLE(BYTE *src, BYTE **dst);

BYTE *back_uncompress_image(BYTE *src)
{
	BYTE *dest;
	BYTE *start;
	WORD i;

//	switch ((*(char *)src) & 0x03f)  // little endian PK 
	switch (((*( (LONG *)src)) >>24)&0x3f)
	{
		...
		case 3:
			start=dest=(BYTE *)idcomp_ptr;
			back_unbetterSimpleRLE(src,&dest);
			idcomp_ptr=(LONG *)(((LONG)(dest+3)) & (~3));
			break;
	}
...
void expand_simpleRLE(BYTE *src, BYTE **dst)
{
	...
	s = (signed char *)src + sizeof(u_long);
	d = (signed char *)(*dst);
	srcBytes = (*(u_long*)src & 0x00FFFFFF) - sizeof(u_long);
	while (srcBytes > 0)
		if ((i = *s++) < 0)
...
/*	static void *table[32] = {&&case2, &&case4, &&case6, &&case8, &&case10, &&case12,
	...
	packdata = src;
	packlen = packdata + (*((u_long*)packdata) & 0x00FFFFFF);
	buffer = (WORD *)(*dst);
...
void unbetterSimpleRLE(BYTE *src, BYTE **dst)
{
	...
	srcBytes = (*(int*)src & 0x00FFFFFF) - (sizeof(int));
