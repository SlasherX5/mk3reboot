...
void smoke_arm(void)
{
	...
	current_proc->a0=current_proc->a9;
...
void smoke_blowup_earth(void)
{
	...
	(PROCESS *)current_proc->pa0=process_exist(PID_MASTER,0xffff);
	((PROCESS *)current_proc->pa0)->procid|=0x8000;
	murder_myoinit_score();

	(PROCESS *)current_proc->pa0=process_exist(PID_MASTER|0x8001,0xffff);
	((PROCESS *)current_proc->pa0)->procid&=(~0x8000);
	bogus_dlist;
...
void mk_game_cabinet(void)
{
	...
	gso_dmawnz(obj,current_proc->a9,obj->oheap,0);
...
void st_spike(void)
{
	...
	current_proc->a11=current_proc->a9;
...
void kabal_scare(void)
{
	...
	current_proc->pa8=pa8;
	current_proc->pa9=ta9;
	(void*)current_proc->pa10=ta10;
	(void*)current_proc->pa11=animframe;
	ochar_sound(9);
	face_ani(3);
...
void face_ani(WORD pa0)
{
	...
	(void *)current_proc->pa11=current_proc->pa9;
	current_proc->pa8=ta8;
	current_proc->pa9=ta9;
	return;
}
...
void kabal_inflator(void)
{
	...
	get_char_ani(ANIM_TABLE2,ANIM_OCHAR_FLOAT);
	takeover_him(pumped);
	(current_proc->pdata.p_otherproc)->a11=current_proc->a9;
...
void pumped(void)
{
	...
	while(current_proc->a9!=current_proc->a10);
...
void jax_grow(void)
{
	...
	current_proc->a9=0x01500150;
	/* jgrow3 */
	do
	{
		obj->oscale.yx=current_proc->a9;

		obj->oxpos.u.intpos=(short)current_proc->pdata.p_store4-get_scaled_x_size(animframe);
		ground_scaled_jax(animframe);

		process_sleep(0x1);
		current_proc->a9-=0x00040004;
	}
	while(current_proc->a9!=0x00500050);
	...
	current_proc->a9=ground_y-objhim->osize.u.ypos;
	objhim->oypos.u.intpos=current_proc->a9-(SCRBOT-SCY(0x20)-32);
	...
	while(objhim->oypos.u.intpos<current_proc->a9);
	stop_a8(objhim);
	objhim->oypos.u.intpos=current_proc->a9;
...
void ground_scaled_jax(OIMGTBL *pa5)
{
	...
	ta3=((current_proc->a9) & 0xffff0000)>>16;
	ta1/=ta3;
...
extern WORD JX2HED_P[];

void get_headsmashed(void)
{
	death_scream;
	create_fx(FX_STAND_BLOOD);

	ADDRESS* animframe = (ADDRESS*)get_char_ani(ANIM_TABLE2, ANIM_OCHAR_CUTUP);
	animframe += (current_proc->pdata.p_otherguy)->ochar;
	current_proc->pa9 = COMPUTE_ADDR((current_proc->pa8)->oheap, GET_LONG(animframe));
	swpal(JX2HED_P, current_proc->pa8);
	mframew(0x20);
	//create_fx(FX_SPIKE_BLOOD);
	process_sleep(0x60);	
	mframew(10);
...
void cutup_correct_pal(void)
{
	...
	switch (obj->ochar)
	{
		case FT_JAX:
			...
			break;
		case FT_SMOKE:
				swpal(SMCUT_P,current_proc->pa8);
			break;
		case FT_LAO:
			...
			break;
		case FT_ROBO2:
				swpal(CYCUT_P,current_proc->pa8);
			break;
	}
...
void lia_scream_rip(void)
{
	...
	current_proc->a11=current_proc->a9;		// save  ochar table
...
void r_scream_ripped(void)
{
	...
	if (obj->ochar==FT_LIA && obj->oid==OID_P2)
		(ADDRESS*)current_proc->pa9=((ADDRESS *)current_proc->a11+15);
...
void sz_lift_n_freeze(void)
{
	...
	animate_a0_frames(8,2);				// reach out
	wfe_him;
	make_him_face_me();
...
void skel_fire_proc(void)
{
	...
	mframew(5);
	current_proc->a10=current_proc->a9;
...
void kano_skeleton(void)
{
	...
	current_proc->a9=current_proc->a0;
...
void fat_robo_crush(void)
{
	...
	(OBJECT *)current_proc->pdata.v_store4=current_proc->pa8;

	get_char_ani(ANIM_TABLE2,ANIM_ROBO_CRUSH_ARMS);
	do_next_a9_frame(current_proc->pa8);
	process_sleep(6);

	(OBJECT *)current_proc->pa10=current_proc->pa8;				// save me here
	...
	(void *)current_proc->pdata.v_store5=current_proc->pa9;
	process_sleep(0x10);
	...
	(void *)current_proc->pdata.v_store7=current_proc->pa9;		// save blood ani

	ptemp=gmo_proc((current_proc->pa8)->oheap);
	a8_front_plus_1(current_proc->pa8);
	(OBJECT *)current_proc->pa0=current_proc->pa8;
	...
	current_proc->pa8=(OBJECT *)(current_proc->pdata.p_store4);		// me
	(OBJECT *)current_proc->pa10=current_proc->pdata.p_slave;					// arm obj (slave)
