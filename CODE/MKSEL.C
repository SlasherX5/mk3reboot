...
******************************************************************************/
extern char* MapStartKeyToString(int player);
...
//***************************************************************************
	...
	p1_char=p2_char=0xffff;			// null characters
	
	/* display mugshots */
	mptr=mug_shot_table;
	(OBJECT *)current_proc->pa9=obj_free;
	(OBJECT *)current_proc->pdata.v_store1=obj_free;		// last object save here
...
/*
	{
		char *str;
		char buffer[40];
		str = MapStartKeyToString(1);
		if ( str )
		{
			strcpy(buffer,"PRESS ");
			strcat(buffer,str);
			strcat(buffer, " TO SELECT PLAYER 2" );
		}
		else
		{
			strcpy(buffer,"NO PLAYER 2 START - KONFIGURE KEYBOARD");
		}
		pds_centered(buffer,SCX(0xc8),SCY(0xec)+26);
	}
...
void p1_select(void)
{
	...
	(PROCESS *)current_proc->pdata.v_store2=ptemp;		// save cursor flash
	...
	(WORD*)(current_proc->pdata.p_myver) = &p1_version;	// pointer to my char
	(WORD *)(current_proc->pdata.p_hischar)=&p2_char;	// pointer to his char
	(WORD*)(current_proc->pdata.p_hisver) = &p2_version;	// pointer to his char
...
void p2_select(void)
{
	...
	(WORD*)(current_proc->pdata.p_myver) = &p2_version;	// pointer to my char
	(WORD *)(current_proc->pdata.p_hischar)=&p1_char;	// pointer to his char	
	(WORD*)(current_proc->pdata.p_hisver) = &p1_version;	// pointer to his char
...
void curs3(WORD mug_index)
{
	...
	*((WORD*)current_proc->pdata.p_myver) = mug_shot_table[mug_index].m_version;

	(OBJECT *)current_proc->pa9=obj_free;
	(OBJECT *)current_proc->pa10=obj_free;
	...
	while (TRUE)
	{
		...
		if ((jpad=JOYSTICK_IN_A0(current_proc))!=0 && current_proc->a10==0) 	// any joy movement and debounce speed
		{
			...
			current_proc->a1 =
				*((WORD*)(current_proc->pdata.p_mychar)) |
				(*((WORD*)(current_proc->pdata.p_myver))) << 8;// get current ochar
...
void cmove9(void)
{
	...
	(WORD *)current_proc->pa2=((WORD *)current_proc->pdata.p_mychar);		// my choice
	...
	do
	{
		...
		(void *)current_proc->pa4=(void *)(((OBJECT *)current_proc->a1)->oshape);
		(void *)current_proc->pa2=((OBJECT *)current_proc->a1)->oslink;
	}
	...
	do
	{
		...
		(OIMGTBL *)current_proc->pa0=((OIMGTBL *)((current_proc->pa8)->oimg))+1;
...
WORD a0_ochar_to_position(WORD ochar)
{
	...
	while (mug_shot_table[loop].m_char != (ochar&0x3f) || 
		mug_shot_table[loop].m_version != (ochar>>8)) {
		loop++;
	}
...
void new_cursor_calla(WORD *tbl,short xoff,short yoff)
{
	...
	/* skip center spot */
	if (current_proc->a1==7 && 0)// && f_smoke==0)
	{
		if (tbl==up_change)
		{
			yoff<<=1;				// double dy
	...
	*((WORD*)current_proc->pdata.p_myver) = mug_shot_table[current_proc->a1].m_version;
...
void psd4(WORD pchar)
{
	...
	get_player_palette(character_palettes_1[obj->over]);
	insert_object(obj,&objlst);
	stance_setup();
		...
		if (obj->oid==2)			// player 2 check pal
		{
			...
			if ((obj->oflags2 & M_UGLY_PAL) == 0)
			{
				if (p1_char==obj->ochar) 		// player 1 same char as me ?, yes
				{
					...
					/* psd9 */
					player_swpal(current_proc,character_palettes_2[obj->over][obj->ochar], obj);
				}
			}
			else
			{
				...
				{
					obj->oflags2&=(~M_UGLY_PAL);
					player_swpal(current_proc,character_palettes_1[obj->over][obj->ochar],obj);
				}
			}
		}
	}
...
void auto_player_select(void)
{
	...
	current_proc->a0 = 15;// (f_smoke == 0) ? 14 : 15;

	(BYTE *)current_proc->pa11=(current_proc->procid==PID_P1SEL) ? p1_movements[randu(current_proc->a0)-1] : p2_movements[randu(current_proc->a0)-1];

	/* aps3 */
	while (TRUE)
	{
		(BYTE)current_proc->a0=*((BYTE *)current_proc->a11);
		((BYTE *)current_proc->pa11)++;
