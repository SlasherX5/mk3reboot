...
void single_pal_fader(void)
{
	...
	l=(current_proc->a9 - getClut(FORE_CLUT_START_X,FORE_CLUT_START_Y))>>CLUT_ID_SHIFT;
	if (l>=NUM_PALS) LOCKUP;
	palptr=palram+l;

	do
	{
		newfade((WORD *)(&(current_proc->Stack)),palptr->palid);
		palset_xfer(FORE_CLUT_START_X+((l%CLUT_FACTOR)<<CLUT_SHIFT_SIZE),
		FORE_CLUT_START_Y+(l/CLUT_FACTOR),(WORD *)(&(current_proc->Stack)));
		process_sleep(current_proc->a8);
	}
...
void white_2_blax(void)
{
	...
	do
	{
		...
		process_sleep(1);	//WIN95: ADDIDTION
		ta10-=2;
	}
	while(ta10>=0);
...
void fast_fadein_jsrp(WORD pa10)
{
	...
	fade_in_fast();
	process_sleep(1);	//WIN95: SUPER HACK!!!!	// allow time for process to die out
	DISPLAY_ON;

	if ( pa10 > 1 );
		fade_sleep_a0(pa10-1);
...
//	DISPLAY_ON;
	...
	process_sleep(2); //WIN95: PREVENT SCREEN FLASH (3) to work!!!!
	DISPLAY_ON;
...
/******************************************************************************
 ...
 Description: 	fadeout
******************************************************************************/
static void KludgePicFade(void);
...
PROCESS *ptemp;
	...
	if(0)///Emu.kludgeflags & KLUDGE_DRAWPIC)
	{
		ptemp = CREATE(PID_FADE,KludgePicFade);
			(WORD *)(ptemp->pa11)=fadeout_tab;
	}
	return;
}
...
void fadeproc(void)
{
	...
	/* fadeplp */
	while ((ta2=GET_WORD(current_proc->pa11)++)!=0xffff)
	{
		/* use bottom of stack for temp storage area for palettes */

		fadepal((WORD *)(&(current_proc->Stack)),(WORD *)current_proc->pa8,ta2);
		palset_xfer(FORE_CLUT_START_X+((l%CLUT_FACTOR)<<CLUT_SHIFT_SIZE),FORE_CLUT_START_Y+(l/CLUT_FACTOR),(WORD *)(&(current_proc->Stack)));
		process_sleep(1);
	}
...
static void KludgePicFade(void)
// fade the palette on the kludgepic
{
WORD ta2;
///KludgePic *pic;
	...
	while ((ta2=GET_WORD(current_proc->pa11)++)!=0xffff)
	{
		///if(!(Emu.kludgeflags & KLUDGE_DRAWPIC))	break;

		///pic = (KludgePic *)player_heap;
		
		///fadepal(&((pic->pclut)[-2]),&((pic->psxclut)[-2]),ta2);
		///Emu.ConvertPalettePSX(pic->pclut,pic->pclut,256); 
		process_sleep(1);
	}
	process_sleep(1);			// give last xfer a chance to go
	process_suicide();
