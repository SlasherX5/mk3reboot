...
PALQENT paltrans_queue[PALQ_MAX_ENTRY];	/* transfer queue */
PALQENT* paltrans_start=0;					/* start ptr in transfer que */
PALQENT* paltrans_end=0;					/* end ptr in transfer que */
PALINFO palram[NUM_F_PALS + NUM_B_PALS];
...
{
	//WIN95 problem with bad pallets????
	//if ( clut < 0x400000) return;

	paltrans_end->csx=cx;
	paltrans_end->csy=cy;
 	paltrans_end->count=GET_LONG(clut)++;
	paltrans_end->csrc=(unsigned short*)clut;

	if ( 0 > paltrans_end->count || 256 < paltrans_end->count )
		return;
...
WORD get_fore_pal(WORD *pal_id)
{
	...
	while (l<NUM_F_PALS)
	{
...
#endif
		{
			...
			cx = FORE_CLUT_START_X + ((l % CLUT_FACTOR) << CLUT_SHIFT_SIZE);
			cy = FORE_CLUT_START_Y + (l / CLUT_FACTOR);
			palset_xfer(cx,cy,pal_id);
			palptr->palcnt++;				/* inc # of objects using this palette */
...
void transfer_palettes(void)
{
	...
	{
		if(((u_short)(paltrans_start->count)) > 256)
			continue;

		LoadPaletteTex((u_short *)paltrans_start->csrc,paltrans_start->count,
					   paltrans_start->csx, paltrans_start->csy );
