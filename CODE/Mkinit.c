...
******************************************************************************/
// defines a modified playstation call to draw with returned error value 
extern int mod_DrawOTag( void *ot );
int iWinActiveState = 1;
int iWinExit = 0;
...
void MK3_Init()
{
	...
	//BuildSoundFiles();
		...
		//WIN95 Allocate heaps
#ifdef FORCE_ALLOCATES
#endif
...
WARMBOOT:
	...
	// Load permanet sounds 
	//PsxSoundLoadDataLoadPerm(20);
...
#ifndef BLOCK_FPS
	fpsbuffer = NULL;
	CREATE(0x8fff, FPS_Display);
#endif

	frametime = Win95_timeGetTime();	
}
...
void MK3_Update()
{
	...
	// Hack to find end of REDBOOK track
	psxcd_loop_callback();
	// Initilize frame clock 	
...
int MK3_Render(int desiredFPS)
{
	...
	if (dlists != NULL)
	{
		...
		// Sync game to 60hertz clock
		retval = 0;

		Win95_Pump();

		curtime = Win95_timeGetTime();		// get real time 

		if (desiredFPS)
		{		                      
			int ddelay = 1000 / desiredFPS;

			while ((curtime - frametime) < ddelay)
			{
				Win95_Pump();
				curtime = Win95_timeGetTime();		// get real time 
			}
		...
		/* update buffers and start draw */
		if ( irqskye & 0x8000 )
		{
			setRGB0(&cdb->clear,GET_RED(irqskye)|0x80,GET_GREEN(irqskye)|0x80,GET_BLUE(irqskye)|0x80);
		}
		else
		{
			setRGB0(&cdb->clear,GET_RED(irqskye),GET_GREEN(irqskye),GET_BLUE(irqskye));
		}
		...
		{			
			///if (f_auto_erase)
			///	retval = mod_DrawOTag(cdb->ot);				/* draw */
			///else 
			///	retval = mod_DrawOTag(cdb->ot+1);			/* draw w/no clear */
		...
		if ( !retval )
		{
			...
			//**** debuging only 
#ifndef BLOCK_FPS
			fpsbuffer = Win95_FPS( skip_cnt, FALSE );
#endif
			skip_cnt = 0;
		}
...
Win95End:
	...
	mk_vblank_int();		// Force V-BLANK and tick increment
	return 0;
}
...
void main_init(void)
{
	...
	curback=9;							/* set current background to 0 */
...
//WIN95:	f_shang_morph=MORPH_QUASI;
