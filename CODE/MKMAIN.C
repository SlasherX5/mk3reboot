...
void force_char(void)
{
// Not used or working bogus
	return;
}
...
void ReadHighScores(int fileno)
{
	read(fileno,factory_initials,sizeof(factory_initials));
}

void WriteHighScores(int fileno)
{
	write(fileno,factory_initials,sizeof(factory_initials));
}
...
void start_entry(WORD *pstate,WORD poffset)
{
	...
	switch (gstate)
	{
		...
		case 0x01: 							// 1 - attract mode
		case 0x0b:							// b - game over start!!
			/* amode_game_start */
			...
			*/
			...
			battle_num=1;					/* set for battle 1 */

			/* MUST FALL INTO do_pstate_jump case */

		case 0x03:							// 3 - buyin
		case 0x04:							// 4 - fighter select
			/* do_pstate_jump */
			switch (*pstate)
			{
				case 0x01:					// 1 - active
				...
				case 0x00:					// 0 - not in game
					/* start_check */
					start_a_game(pstate,poffset);
					break;
				...
				case 0x03:					// 3 - auto select check
					if (&p1_state==pstate)
					{
						current_proc->a5=MASK_JUP;		// p1 up mask
						current_proc->a6=FT_ST;
						current_proc->a0=PID_P1SEL;
					}
					else
					{
						current_proc->a5=MASK_JUP<<16;		// p2 up mask
						current_proc->a6=FT_LK;
						current_proc->a0=PID_P2SEL;
					}

					if ((((PROCESS *)(current_proc->pa0))=process_exist(current_proc->a0,-1))==NULL)		// do i have a cursor process
						process_suicide();
...
#if 0
/******************************************************************************
 Function: void endurance_round(void)
 ...
 Description:	play an endurance round
******************************************************************************/
...
psxcd_stop();
	...
	if (f_smoke!=0)
	{
		...
		//play_generic_tune(TRK_SOUL);				// SCA3(0x21)
	...
	arcade_page();					// show an arcade code
#endif
...
WORD play_1_round(void)
{
	...
	{
		curback=BKGD_TOWER_MOD;		// play on motaros turf
	}
	else
	{
		if (p1_char==FT_SK || p2_char==FT_SK)
		{
			curback=BKGD_MOCKPIT_MOD;		// play on pit
		}
		else
		{
			
			if (p1_char==FT_NOOB || p2_char==FT_NOOB)
			{
				curback=BKGD_NOWALL_MOD;		// play on nowall balcony
			}
		}
	}

	/* playn */
	if (f_smoke==0) 			// adjustment is not set - this is special
	{
		if (p1_char==FT_SMOKE || p2_char==FT_SMOKE)
		{
			curback=BKGD_HIDDEN_MOD;		// play on hidden
		}
	}

	/* playm */
	do_background(curback);				// -VSON- need to adjust for process sleep
...
//	psxcd_play_at((f_music==0)?TRK_FIGHT:get_background_tune(),cd_vol_tbl[f_music],(f_no_sfx==0)?2:300);
	if (f_music)
		play_generic_tune(get_background_tune());
		//psxcd_play_at_andloop(get_background_tune(),cd_vol_tbl[f_music],0,0,get_background_tune(),cd_vol_tbl[f_music],450,3000);
...
CLOCK5:
		if (clk_ones+clk_tens!=0)
		{
			(LONG)current_proc->pa8=CLOCK_SPEED;					/* clock speed counter */
			do
			{
CLOCK4:
				...
				if (p1_bar==0 || p2_bar==0)
				{
					...
					if (p1_bar>p2_bar)						/* p1 or p2 won */
						return(ROUND_P1_WON);
					else return(ROUND_P2_WON);
				}
...
void results_of_round(WORD results)
{
	if (results==ROUND_TIMEOUT)
	{
		...
		if (p1_bar==p2_bar)						/* tie game */
			results=ROUND_P1_P2_TIE;
		else results=(p1_bar>p2_bar) ? ROUND_P1_WON:ROUND_P2_WON;		/* p1 or p2 won */
	}
...
//		else 
//			psxcd_play_at_andloop(get_background_tune(),cd_vol_tbl[f_music],(f_no_sfx==0)?2:300,0,get_background_tune(),cd_vol_tbl[f_music],450,3000);
...
WORD set_drone_ochar(void)
{
	...
	if (pchar == &p1_char && ta0 == 2) p1_version = 1;
	if (pchar == &p2_char && ta0 == 2) p2_version = 1;
...
//WIN95: Force charater
//	return FT_SK;
//	return FT_MOTARO;
	return(*(map_start+map_position));		// next "enemy offset" to battle with
}
...
{
	curback=(curback<MAX_BACKGROUNDS-1) ? curback+1:0;		// match over --> next bkgnd
...
******************************************************************************/
WORD mhe_minimums_1[]={0,1,0};
WORD mhe_minimums_2[]={1,2,0};
WORD mhe_minimums_3[]={1,3,0};
WORD mhe_minimums_4[]={3,4,2};
WORD mhe_minimums_5[]={4,5,3};

WORD *game_adjustment_minimums[]=
{
	mhe_minimums_1,
	mhe_minimums_2,
	mhe_minimums_3,
...
void diff_adjust(void)
{
	...
	/* dadj5 */
	adjtbl=(short *)game_adjustment_minimums[get_adj_diff()];

	/* ladder_order_a1 */
	current_proc->a1=ladder_order;
	if (current_proc->a1>=3)
		current_proc->a1=1;

	if (*(adjtbl+current_proc->a1)>diff)
		diff=*(adjtbl+current_proc->a1);		// minimum diff level

	/* stuff_a0_diff */
	if (diff>9)
		diff=5;				// to big=just use 5
...
******************************************************************************/
 short get_adj_diff_word(short *difftbl)
{
...
/******************************************************************************
 Function: short add_to_diff(short deltadiff)

 By: David Schwartz

 Date: Oct 1994

 Parameters: deltadiff - amount to add

 Returns: None

 Description:	add a value to the variable @diff
******************************************************************************/
void add_to_diff(short deltadiff)
{
	diff=diff+deltadiff;				// set diff

	if (diff<0) diff=0;					// bounds check min

	if (diff>9) diff=9;					// bounds check max
...
void finish_him_or_her(void)
{
	...
	pchar=(p1_bar==0)? &p1_char : &p2_char;
...
{
	if (curback>=BKGD_MK3_BUYIN)
		curback=BKGD_SUBWAY_MOD;
	return;
}
...
******************************************************************************/
WORD TRK_HIDDEN[] = { TUNE_OFFBEAT1,TUNE_OFFBEAT2,TUNE_OFFBEAT3,TUNE_OFFBEAT4,0 };
WORD TRK_SOUL[]   = { TUNE_FRENZY1A,TUNE_FRENZY2,TUNE_FRENZY3,TUNE_FRENZY1B,TUNE_FRENZY2,TUNE_FRENZY3,TUNE_FRENZY1C,TUNE_FRENZY2,TUNE_FRENZY3,TUNE_FRENZY1D,TUNE_FRENZY2,TUNE_FRENZY3,0 };
WORD TRK_OLD[] = { TUNE_OLD1,TUNE_OLD2,TUNE_OLD3,TUNE_OLD4,TUNE_OLD5,TUNE_OLD6,TUNE_OLD7,TUNE_OLD8,0 };
WORD TRK_COOLT[] = { TUNE_COOLT1,TUNE_COOLT2,TUNE_COOLT3,TUNE_COOLT4A,TUNE_COOLT5,TUNE_COOLT5,TUNE_COOLT6,TUNE_COOLT6,4 };
WORD TRK_TRAIN[] = { TUNE_TRAIN1,TUNE_TRAIN2,TUNE_TRAIN3A1,TUNE_TRAIN4,TUNE_TRAIN5,0 };

WORD round_1_tunes[][16] =
{
	{TUNE_TRAIN1,TUNE_TRAIN2,TUNE_TRAIN3A1,TUNE_TRAIN4,TUNE_TRAIN5,0},
	{TUNE_OLD1,TUNE_OLD2,TUNE_OLD3,TUNE_OLD4,TUNE_OLD5,TUNE_OLD6,TUNE_OLD7,TUNE_OLD8,0},
	{TUNE_SLOW3,TUNE_SLOW4,TUNE_SLOW5,TUNE_SLOW6,TUNE_SLOW7,TUNE_SLOW8,0},
	{TUNE_COOLT1,TUNE_COOLT2,TUNE_COOLT3,TUNE_COOLT4A,TUNE_COOLT5,TUNE_COOLT5,TUNE_COOLT6,TUNE_COOLT6,4},
	{TUNE_PIT1,TUNE_PIT2,TUNE_PIT3,TUNE_PIT4,TUNE_PIT5,0},
	{TUNE_WACKER1A,TUNE_WACKER2A,TUNE_WACKER3,TUNE_WACKER4,TUNE_WACKER1B,TUNE_WACKER2A,TUNE_WACKER3,TUNE_WACKER4A,0},	
	{TUNE_TONGUE1,TUNE_TONGUE2A,TUNE_TONGUE3,TUNE_TONGUE4,TUNE_TONGUE5,TUNE_TONGUE4,TUNE_TONGUE5,1},
	{TUNE_SLOW3,TUNE_SLOW4,TUNE_SLOW5,TUNE_SLOW6,TUNE_SLOW7,TUNE_SLOW8,0},
	{TUNE_CHURCH1,TUNE_CHURCH2,TUNE_CHURCH3,TUNE_CHURCH4,2},
	{TUNE_GRAVE1,TUNE_GRAVE2,TUNE_GRAVE3,TUNE_GRAVE4,TUNE_GRAVE5,0},
	{TUNE_PIT1,TUNE_PIT2,TUNE_PIT3,TUNE_PIT4,TUNE_PIT5,0},
	{TUNE_FRENZY1A,TUNE_FRENZY2,TUNE_FRENZY3,TUNE_FRENZY1B,TUNE_FRENZY2,TUNE_FRENZY3,TUNE_FRENZY1C,TUNE_FRENZY2,TUNE_FRENZY3,TUNE_FRENZY1D,TUNE_FRENZY2,TUNE_FRENZY3,0},
	{TUNE_OFFBEAT1,TUNE_OFFBEAT2,TUNE_OFFBEAT3,TUNE_OFFBEAT4,0},
	{TUNE_OFFBEAT1,TUNE_OFFBEAT2,TUNE_OFFBEAT3,TUNE_OFFBEAT4,0},
	{TUNE_OFFBEAT1,TUNE_OFFBEAT2,TUNE_OFFBEAT3,TUNE_OFFBEAT4,0},
	{TUNE_OFFBEAT1,TUNE_OFFBEAT2,TUNE_OFFBEAT3,TUNE_OFFBEAT4,0},
	{TUNE_OFFBEAT1,TUNE_OFFBEAT2,TUNE_OFFBEAT3,TUNE_OFFBEAT4,0},
...
#define BKGD_HIDDEN_MOD		0x0b
*/

WORD finish_tunes[][16] =
{
	{TUNE_TRAINFIN,0},				// BKGD_SUBWAY_MOD		0x00
	{TUNE_OLDFIN,TUNE_OLD2,	0},		// BKGD_STREET_MOD		0x01
	{TUNE_SLOWFIN,	0},				// BKGD_BANK_MOD		0x02
	{TUNE_COOLTFIN,0},				// BKGD_ROOF_MOD		0x03
	{TUNE_PITFIN,		0},		    // BKGD_TOWER_MOD		0x04
	{TUNE_WACKFIN,TUNE_WACKER4,	0},	// BKGD_BRIDGE_MOD		0x05
	{TUNE_TONGFIN,	0},				// BKGD_SOUL_MOD		0x06
	{TUNE_SLOWFIN,	0},				// BKGD_BANK_MOD		0x02
	{TUNE_CHRCHFIN,0},				// BKGD_TEMPLE_MOD		0x08
	{TUNE_GRAVEFIN,0},				// BKGD_GRAVE_MOD		0x09
	{TUNE_PITFIN,		0},			// BKGD_PIT_MOD		0x0a
	{TUNE_PITFIN,		0},		// B - HIDDEN
	{TUNE_PITFIN,		0},		// C - HIDDEN
	{TUNE_PITFIN,		0},		// D - HIDDEN
	{TUNE_PITFIN,		0},		// E - HIDDEN
	{TUNE_PITFIN,		0},		// F - HIDDEN
	{TUNE_PITFIN,		0},		// 10 -	HIDDEN
	{TUNE_PITFIN,		0},		// 11 -	HIDDEN
	{TUNE_PITFIN,		0},		// 12 -	HIDDEN
	{TUNE_PITFIN,		0},		// 13 -	HIDDEN
	{TUNE_PITFIN,		0},		// 14 -	HIDDEN
};
...
WORD* get_background_tune(void)
{
	...
	else
	{
		...
		{
			return(round_1_tunes[BKGD_PIT_MOD]);
		}
		else
		{
			if (round_num==1)
				return(round_1_tunes[curback]);
			else return(round_23_tunes[curback]);
		}
	}
}
...
void start_tune(void)
{
	...
	if ( winner_status!=3 && f_death==0)
	{
		...
		//psxcd_play_at(get_background_tune(),0,1100);

		play_generic_tune(get_background_tune());
		//psxcd_play_at_andloop(get_background_tune(), cd_vol_tbl[f_music], 1100, 2500, get_background_tune(), cd_vol_tbl[f_music], 450, 3000);
	}
	process_suicide();
}
...
{
	{0x100000,bc_throwing_disabled},
	{0x200000,bc_blocking_disabled},
	{0x300000,bc_p1_handicap},
	{0x400000,bc_p2_handicap},
	{0x500000,bc_dark_fighting},
	{0x600000,bc_switcheroo},
	{0x700000,bc_jackbot},
	{0x800000,bc_p1_supercap},
	{0x900000,bc_p2_supercap},
	{0x010000,bc_space_game},
	{0x020000,bc_wf_motaro},
	{0x030000,bc_wf_sk},
	{0x040000,bc_wf_noob},
	{0x050000,bc_nofear_clue},
	{0x060000,bc_no_powerbars},
	{0x070000,bc_uppercut_recovery},
	{0x080000,bc_unlim_run},
	{0x090000,bc_psycho_kombat2},
	{0x001000,bc_intro_message},
	{0x002000,bc_wf_smoke},
	{0x003000,bc_no_combos},
	{0x004000,bc_superjumps},
	{0x005000,bc_no_special_moves},
	{0x006000,bc_p1_half_damage},
	{0x007000,bc_p2_half_damage},
	{0x008000,bc_p1p2_half_damage},
	{0x009000,bc_bar_regenerate},
	{0x000100,bc_super_endur},
	{0x000200,bc_real_kombat},
	{0,NULL}
};
#else
BCC brief_case_codes[]=
{
	...
	{0x020020,bc_blocking_disabled},
	{0x033000,bc_p1_handicap},
	{0x000033,bc_p2_handicap},
	{0x688422,bc_dark_fighting},
	{0x460460,bc_switcheroo},
	{0x987666,bc_jackbot},
	{0x707000,bc_p1_supercap},
	{0x000707,bc_p2_supercap},
	{0x642468,bc_space_game},
	{0x969141,bc_wf_motaro},
	{0x033564,bc_wf_sk},
	{0x769342,bc_wf_noob},
	{0x282282,bc_nofear_clue},
	{0x987123,bc_no_powerbars},
	{0x688433,bc_uppercut_recovery},
	{0x466466,bc_unlim_run},
	{0x985125,bc_psycho_kombat2},
	{0x123926,bc_intro_message},
	{0x205205,bc_wf_smoke},
	{0x722722,bc_no_combos},
	{0x321789,bc_superjumps},
	{0x555556,bc_no_special_moves},
	{0x390000,bc_p1_half_damage},
	{0x000390,bc_p2_half_damage},
	{0x390390,bc_p1p2_half_damage},
	{0x975310,bc_bar_regenerate},
	{0x024689,bc_super_endur},
	{0x040404,bc_real_kombat},
	{0,NULL}
};
#endif

/******************************************************************************
 Function: void try_briefcase(void)
