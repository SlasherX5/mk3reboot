...
static OBJECT object_ram[NUM_OBJECTS];		/* object storage ram */

#undef MEM_OBJECTS
#ifdef MEM_OBJECTS // memory objects

#define OPTR(p,o) ((unsigned char*)(p)+o)

static void **obj_mem_list = NULL;
extern void *malloc();

static int free_all_mem_objects(void)
{
OBJECT *obj;

	while((obj = (OBJECT *)obj_mem_list) != NULL)
	{
	 	obj_mem_list = *(void **)obj;
	 	free(obj);
	}
};
static void free_mem_object(OBJECT *obj)
{
void **pobj = (void **)&obj_mem_list;
void **node;

	obj = (OBJECT *)OPTR(obj,-sizeof(void **));	// compensate for list node

	while((node = (void **)(*pobj)) != NULL)
	{
	 	if(obj == node)
		{
			*pobj = *node;
...
static OBJECT *alloc_mem_object()
{
void **obj;

	if((obj = (void **)malloc(sizeof(OBJECT) + sizeof(void**))) != NULL)
	{
  		obj[0] = obj_mem_list[0];
		obj_mem_list[0] = (void *)obj;
		obj = OPTR(obj,sizeof(void **));
	}
	return((OBJECT *)obj);
}
#endif // memory objects
...
{
	///Emu.kludgeflags = 0;   // ahh the kludge!
	...
	// free any allocated objects that have not yet been freed
...
// object must not be attached to a list when this is called
{
	dealloc_vram_mem(obj->ovcache);

#ifdef MEM_OBJECTS
	// if not within object ram area free dynamicly allocated object
	if(obj < &object_ram[0] || obj >= &object_ram[ArrayCount(object_ram)])
	{
	 	free_mem_object(obj);
		return;
	}
	...
	obj->ovcache=VCACHE_FAILED;
	obj->olink=obj_free;		/* insert killed node at the top of the free list */
	obj_free=obj;
}
...
#ifdef MEM_OBJECTS
	else if((new_obj = alloc_mem_object()) == NULL)
		goto error;
#endif
...
/******************************************************************************
 ...
 Description:	sets to zero or NULL all longs
******************************************************************************/
extern int kludge_death_count;
extern int kludge_smoke_spear;
extern int kludge_obj;
extern int kludge_obj_pos;
 ...
 *****************************/
				if (!(oflags2 & M_INVISO))
				{
					...
					while (pram_ptr->prsagptr != 0)
					{
						...
						if (object_ptr->oscale.u.xpos != 0x0100) scx = (scx << 8) / object_ptr->oscale.u.xpos;
						if (object_ptr->oscale.u.ypos != 0x0100) scy = (scy << 8) / object_ptr->oscale.u.ypos;

						setXY4(cdb->curr_prim,
							objpos.u.xpos - pram_ptr->prpos.u.xpos,
							objpos.u.ypos - pram_ptr->prpos.u.ypos,
							scx, scy,
							(object_ptr->oflags & (M_FLIPH + M_FLIPV)) >> 4);
						...
						SetUVWH(cdb->curr_prim,object_ptr->header.t_xoffset,	\
								...
								object_ptr->bmp ? object_ptr->bmp[0] : object_ptr->header.t_width, \
								object_ptr->bmp ? object_ptr->bmp[1] : object_ptr->header.t_height);
...
void delete_string(int stringid,OBJECT **olist)
{
OBJECT *obj_ptr;
OBJECT *obj_prev;
		...
		while (pram_ptr->prsagptr != 0)
		{
			...
			setXY4(cdb->curr_prim,
				(objpos.u.xpos - pram_ptr->prpos.u.xpos), ypos - dist,
				 obj->header.t_width, (obj->header.t_height >> 2),
				(obj->oflags & (M_FLIPH + M_FLIPV)) >> 4);
			...
			*/
			SetUVWH(cdb->curr_prim, obj->header.t_xoffset, \
				...
				obj->bmp ? obj->bmp[0] : obj->header.t_width, \
				obj->bmp ? obj->bmp[1] : obj->header.t_height);
...
void floor_code(void)
{
	...
	sprt->tpage = control_floor_tpage;
	sprt->u=(BYTE)(FLR_L_BASE_X%128)/2;
	sprt->v=(BYTE)(FLR_L_BASE_Y%256);
	sprt->w=(BYTE)(FLR_BUFFER_X);
	sprt->h=(BYTE)(skew_height-1+12);
	sprt->x=0;
	sprt->y=(skew_y-worldtly.u.intpos);
   	SetSemiTrans(sprt, 0);
	cdb->primcnt++;
	...
	sprt->clut=skew_constpal;
	sprt->tpage = control_floor_tpage;
	sprt->u=(BYTE)(FLR_R_BASE_X%128)/2;
	sprt->v=(BYTE)(FLR_R_BASE_Y%256);
	sprt->w=(BYTE)(FLR_BUFFER_X);
	sprt->h=(BYTE)(skew_height-1+12);
	sprt->x=160;
	sprt->y=(skew_y-worldtly.u.intpos);
