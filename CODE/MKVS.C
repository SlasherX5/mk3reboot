...
//WIN95 MOVED HIGHER
//WIN95	dallprc(PID_COMB);				// stop combinations proc
...
//WIN95 MOVED HIGHER UP 
//WIN95:	file_load_message(1);
//WIN95:	process_sleep(2);
	freeze_2_pages();
	murder_myoinit_score();
#endif
...
#if 0 // these are in the overlay bin files and not referenced
VPIECE *ochar_pieces[] =
{
	kano_pieces,
	sonya_pieces,
...
void slide_in_huge_players(void)
{
	...
	//WIN95: SUPER HACK !!!! STOP THE FLASH 
	DISPLAY_OFF;
	process_sleep(3);
	DISPLAY_ON;

	process_sleep(((SDIST/(SSPEED/0x10000))-8)-3);
...
void cd_preload_bkgd(void)
{
	...
	{
		save_curback=BKGD_TOWER_MOD;		// play on motaros turf
	}
	else
	{
		if (p1_char==FT_SK || p2_char==FT_SK)
		{
			save_curback=BKGD_MOCKPIT_MOD;		// play on pit
		}
		else
		{
			if (p1_char==FT_NOOB || p2_char==FT_NOOB)
			{
				save_curback=BKGD_NOWALL_MOD;		// play on nowall balcony
			}
			else
			{
	...
	texture_level_load(table_o_levels[save_curback],ASYNC_LOAD);
	while ( psxcd_async_on() )
		process_sleep(1);
#ifndef BLOCK_FILE_IO
	module_os_close(module_int);
